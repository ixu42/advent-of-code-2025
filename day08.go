...
func getAllBoxPositions(lines []string) error {
	boxes = make([]box, 0)
	for _, line := range lines {
		vals := strings.Split(line, ",")
		if len(vals) != 3 {
			return fmt.Errorf("Invalid line format:", line)
		}
		...
		if errX != nil || errY != nil || errZ != nil {
			return fmt.Errorf("Error converting string to int:", errX, errY, errZ)
		}
...
func solvePart2() int {
	...
	return int(boxes[lastPair[0]].x*boxes[lastPair[1]].x)
}
...
func main() {
	...
	if err := getAllBoxPositions(lines); err != nil {
		fmt.Println("Error getting box positions:", err)
		return
	}

	conns = make([]connection, 0)
	getAndSortConnnections()

	parent = make([]int, len(boxes))
	size = make([]int, len(boxes))
	for i := 0; i < len(boxes); i++ {
		parent[i] = i // each box starts in its own circuit/tree
		size[i] = 1 // each circuit/tree starts with size 1
	}
